        ...
        @if($roles)
            @foreach($roles as $role)
                <tr>
                    ...
                    <td>{{$role['name']}}</td>
                    <td>
                        <a href="{{action('Roles@edit', $role['id'])}}" class="btn btn-default btn-sm btn-rounded" title="Edit role"><span class="fa fa-edit"></span></a>
                    </td>
                    <td>
                        {{-- {!! Form::open(['method'=>'delete', 'url'=> "admin/roles/".$role['id'] ]) !!}
                        {!! Form::hidden('id', $role['id'] ) !!} --}}

                        {{-- <form action="{{ url('admin/roles/'.$role['id'])}}" method="delete">
                        ...
                        <a onclick="return confirm('Are you sure?')" href="{{ url('admin/roles/delete/'.$role->id)}}" class="btn btn-danger btn-sm btn-rounded"><span class="fa fa-times"></span></a>
                        {{-- {!! Form::close() !!} --}}
                    </td>
                    <td>
